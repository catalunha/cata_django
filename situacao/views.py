...
def programanovo(request,id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            novo.indices.add(indice)
            form.save_m2m()
            proposta,criado = Proposta.objects.get_or_create(programa=novo,arquivo='interface.py')
            proposta,criado = Proposta.objects.get_or_create(programa=novo,arquivo='solucao.py')
